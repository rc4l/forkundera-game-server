...
IMAGE="${FUA_IMAGE:-ghcr.io/rc4l/forkundera-game-server:engine-v0.3.2}"
...
# [rc4l] Connection history has to outlive the container. docker logs dies with the container, and
# every update recreates it, so "how many people have joined?" was unanswerable past the last deploy.
#
# Owned by the image's uid because the server writes this as an unprivileged user; a root-owned
# directory here means the engine opens nothing and says nothing about it.
LOGS_DIR="${ROOT}/logs"
mkdir -p "${LOGS_DIR}"
SERVER_UID="$(docker run --rm --entrypoint id "${IMAGE}" -u 2>/dev/null || echo 999)"
case "${SERVER_UID}" in ''|*[!0-9]*) SERVER_UID=999 ;; esac
chown "${SERVER_UID}:${SERVER_UID}" "${LOGS_DIR}" 2>/dev/null || true

{
	...
	# [rc4l] sv_logfilenametimestamp defaults on, so each boot writes its own
	# name__YYYY_MM_DD-HH_MM_SS.log and a restart never truncates what came before.
	echo "    command: [\"+logfile\", \"/data/logs/${INSTANCE}\"]"
	echo "    volumes:"
	echo "      - ${VOLUME}:/data"
...
# [rc4l] PROVE the logfile exists rather than assuming it. This whole feature is a silent one: if the
# mount is wrong or the directory is not writable by the server, the engine carries on perfectly and
# writes nothing, and you find out weeks later when the history you were counting on is not there.
if [ "${ready}" -eq 1 ]; then
	logged=0
	for _ in $(seq 1 10); do
		if ls "${LOGS_DIR}/${INSTANCE}__"*.log >/dev/null 2>&1; then logged=1; break; fi
		sleep 1
	done
	[ "${logged}" -eq 1 ] 		|| warn "no logfile appeared in ${LOGS_DIR} -- connection history is NOT being recorded for ${INSTANCE}"
fi
